...
impl CameraSystem {
    fn update_rotation(&mut self, event: &on::MouseMoved){
        println!("camera update in progress!");
        let device_system = self.ctx.get::<DeviceSystem>();
        let device = &device_system.device;
        let queue = &device_system.queue;
        ...
        let buffer_data: &[u8] = bytemuck::cast_slice::<CameraBuffer, u8>(&binding);

        println!("Writing buffer data: {:?}", bytemuck::cast_slice::<CameraBuffer, u8>(&[self.camera_buffer]));
        ...
        //queue.submit(Some(encoder.finish())); // Important!

        println!("{:?}", std::mem::align_of::<CameraBuffer>());
        //println!("submitted");

        //println!("buffer data: {:?}", bytemuck::bytes_of(&self.camera_buffer));
        //println!("polled");
        // bytemuck::bytes_of(&self.camera_buffer) | for the data to update...
