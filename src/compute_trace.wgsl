...
struct Camera {
    position: vec3<f32>, 
    direction: vec3<f32>,  // 12 bytes  // Manually add padding to align the next field
    fov: f32,
    //padding: f32,  // 12 bytes + 4 bytes padding
    //padding: f32,
};
...
fn cs_main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let ray_dir = generate_ray_direction(camera.direction, uv, camera.fov, aspect);
